...
## 一句话简介
采集 Twitter/X 搜索结果页或话题时间线，自动提取推文内容、作者、互动数据

## 功能说明
- 采集源：`social/twitter-search` 站点适配器（市场 ID：`social/market/twitter-search`）
- 采集方式：浏览器引擎（`browser`），匹配 `*/search*`, `*/hashtag/*`
...
| 占位符 | 必填 | 默认值 | 说明 |
|---|---|---|---|
| search_url | ✓ | — | Twitter 搜索页地址 (如 https://twitter.com/search?q=keyword) |
...
## 使用方式
1. 在 GUI 模板市场安装（信任根验签通过），或 CLI：`omnicrawler templates install social/market/twitter-search`
