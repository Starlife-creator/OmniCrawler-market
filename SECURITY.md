...
- **单信任根 ed25519（维护者轨）**：签名用持有者冷存储的私钥生成（离线机），验签用随包分发
...
仓库内同时存在 **创作者轨** 与 **维护者轨** 两条独立签名链：

- `creator.sig` + `creator.identity`：创作者用本地热密钥（身份库）对
  `plugin.py` / `template.yaml` 签名，公钥指纹记录在 `creator.identity`，
  与 `authors/` 中 `roles: [creator]` 的作者对应。
- 两条轨**各自独立验签**（`generate_catalog.py`）：维护者轨对信任根验签，
  创作者轨对 `creator.identity` 公钥验签，互不替代、可同时存在。
- **创作者轨不构成分发背书**：市场分发路径上，模板强制要求维护者冷密钥签名
  （`template.yaml.sig`），插件要求维护者轨或创作者轨二者至少其一；
  仅创作者签名不等于通过市场审核。
